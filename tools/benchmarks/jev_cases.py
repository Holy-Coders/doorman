...
from evaluate import groups, assign_groups, fingerprint, FEATURES
...
for f,family in enumerate(families):
    ...
    selected.extend(dict(index=i,fold=family,positive=True,features={k: r['features'][k] for k in FEATURES if k in r['features']}) for i,r in candidates[:6 if f<5 else 5])
...
selected.extend(dict(index=i,fold=families[j%len(families)],positive=False,features={k: r['features'][k] for k in FEATURES if k in r['features']}) for j,(i,r) in enumerate(candidates[:40]))
