...
`src/docs.json` defines the reading order, navigation groups, page titles and source files. The documentation sync script builds pages and a local search index from the library's README, reference docs, privacy document, and example guides. Edit those original files; generated copies are ignored by Git. Storage documentation lives in `content/storage.md`.
