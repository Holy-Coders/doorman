...
The home, searchable docs, local examples and opt-in live playground at https://janitor.holycoders.io. Astro builds static pages. The existing `janitor-docs` Worker handles `/api/playground/*` and hourly cleanup; other requests use static assets.

Ordinary page views and local examples collect nothing. The activated live demo uses Janitor's collectors, HTTP handler, real matching engine, D1 storage, request controls and Jev evaluator. Its session-scoped wrapper is in `worker/`. See [playground behavior and privacy](../docs/PLAYGROUND.md).
...
pnpm --filter @janitor/site migrate:local
pnpm --filter @janitor/site run dev:live --local-protocol https
```
...
pnpm site:test
pnpm site:build
pnpm --filter @janitor/site exec wrangler deploy --dry-run
```

## Deploy

`wrangler.jsonc` uses the dedicated `janitor-playground` D1 database. Apply the seven library migrations plus `worker/schema.sql` for demo sessions, ownership, cached answers and budgets. The schema is idempotent; there is no request-time table creation.

```sh
pnpm --filter @janitor/site exec wrangler whoami
pnpm --filter @janitor/site migrate:remote
# Once, enter a dedicated random secret of at least 32 characters when prompted:
pnpm --filter @janitor/site exec wrangler secret put PLAYGROUND_SECRET
pnpm --filter @janitor/site run deploy
```
...
Calls use the account's `default` gateway with per-request `collectLog: false`, `skipCache: true` and `retries.maxAttempts: 1`. These prevent storing prompts in Gateway logs/cache or amplifying one Janitor reservation into multiple provider attempts. Other applications' gateway settings are unchanged.
...
```sh
pnpm --filter @janitor/site exec wrangler d1 execute VISITORS --remote \
...
```sh
pnpm --filter @janitor/site exec wrangler deploy --var JEV_ENABLED:false
```
