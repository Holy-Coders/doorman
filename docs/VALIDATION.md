...
The current TypeScript suite passes **397 tests across 32 files**, including configurable scoring, stored operator policies, optional font/runtime/permission probes, linked API activity, the classifier, external-data projection and budgeted Jev transport checks. The offline benchmark importer/evaluator adds **nine Python tests**, including checks that unseen labels cannot change fitting, model selection or thresholds. Strict typechecking and lint pass. Shared storage checks also passed against a real Postgres 17 server and Cloudflare's local D1 runtime. The public site has seventeen browser tests.

The current native Elixir suite passes **67 tests** against a temporary local Postgres 18.1 database, including the new bounded field validation and font/evaluator projection. Three browser-to-server/analytics integration tests and seventeen public-site browser tests pass. The local experimental probe harness runs Chromium, Firefox and WebKit on one Mac. It checks fixed local-font stability, target/keyboard separation, page-font failure handling, hidden-decoy accessibility-tree exclusion, cleanup and absent APIs. Fifteen screenshot captures produced zero focus/visibility changes. A separately launched no-CDP Chromium control also completed. Both instrumented and uninstrumented runs exposed zero of the sampled legacy marker names; timings overlap and do not establish a CDP detector. [Report and scope](EXPERIMENTAL-DETECTION.md).

The public research datasets were replayed again after this change, and the cached Jev pilot made **zero new calls**. These datasets lack the new probe fields; unchanged benchmark results are regression evidence, not validation of those fields or the new operator/activity prompts. No model was promoted and no paid inference was started.
