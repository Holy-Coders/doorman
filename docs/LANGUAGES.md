...
Doorman 0.12.0 is the current registry release. The new 0.13 identity-context API is available from source as a source preview. Install the JavaScript packages from npm or the native Elixir package from Hex. You do not need a Doorman API key. Optional Jev evaluation uses credentials for your chosen AI provider.
