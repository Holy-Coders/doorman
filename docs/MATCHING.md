...
3. Without a usable cookie, each indexed coarse branch examines at most the newest 50 retained observation rows: `(platform, browser)`, WebGL renderer, and `(timezone, browser)`. Union/deduplication returns at most ten visitor IDs, ordered by activity. No coarse keys means no search.
4. For each candidate load at most five observations. Compare the current observation to each; retain the strongest non-contradictory snapshot score. Historical variability is then judged across the entire compact history by Jev.
...
7. Restore only if the highest score is at least `0.90` and exceeds the next candidate by at least `0.03`. Unevaluated candidates also participate in ambiguity checking using their deterministic scores. Otherwise create a new 192-bit opaque ID.
