"""Call your application's Janitor endpoint. This is not a native matching engine."""
...
class Client:
    ...
    def __init__(self, endpoint: str, *, timeout: float = 3, bearer_token: str | None = None):
        ...
        if (url.scheme not in ("http", "https") or not url.hostname or url.username
            ...
            raise ValueError("Expected an absolute Janitor endpoint without credentials or query")
    ...
    def identify(self, signals: dict, *, behavior: dict | None = None,
        ...
        try:
            ...
            if response.status != 200 or len(raw) > MAX_RESPONSE_BYTES:
                raise JanitorError("Janitor endpoint unavailable")
            if response.getheader("Content-Type", "").split(";")[0].strip() != "application/json":
                raise JanitorError("Invalid Janitor response")
            ...
            if len(cookies) > 8:
                raise JanitorError("Invalid Janitor cookies")
            ...
            raise JanitorError("Janitor endpoint unavailable") from error
        finally:
            connection.close()
