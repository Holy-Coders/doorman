...
export function createPostgresLearningStorage(
  db: PostgresDatabase,
): LearningStorage {
  ...
  return {
    ...
    async updateSession(s, now) {
      await rows(
        "UPDATE learning_sessions SET observed_at = $5, signals_json = $6::jsonb, prediction_status = $7, predicted_subject_id = $8, prediction_score = $9 WHERE scope = $1 AND id = $2 AND started_at = $3 AND expires_at = $4 AND expires_at > $10 AND subject_id IS NULL AND disputed = 0",
        [...values(s), now],
      );
    },
    async confirmSession(scope, id, subjectId, now) {
      ...
      await rows(
        "DELETE FROM learning_sessions WHERE scope = $1 AND subject_id = $2 AND id IN (SELECT id FROM learning_sessions WHERE scope = $1 AND subject_id = $2 ORDER BY verified_at DESC, id DESC OFFSET 20)",
        [scope, subjectId],
      );
    },
