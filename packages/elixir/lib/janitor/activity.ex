defmodule Janitor.Activity do
  ...
  defp evaluate(c, context, owner) do
    ...
    lease = Janitor.random_id("activity_")
    ...
    if Store.claim(c, id, owner, lease, now, expiry) do
      ...
      fun =
        cond do
          ...
          is_list(c.evaluator) -> fn -> Janitor.Jev.evaluate_activity(input, c.evaluator) end
          true -> nil
        end

      protection =
        c.protection || Janitor.Protection.configure(c.identity ++ [evaluator: [max_calls: 60]])

      result =
        if fun && summary["buckets"] != [],
          do: Janitor.Protection.evaluate(%{c | protection: protection}, fun, &valid_risk?/1),
          else: nil
  ...
  defp bounded(c, fun) do
    case Janitor.Bounded.run(fun, c.activity.timeout_ms) do
      {:ok, result} -> result
  ...
  defp owner(c, key),
    do: Janitor.Identity.label(c.identity, Jason.encode!(["api-activity-v1", key.kind, key.id]))
