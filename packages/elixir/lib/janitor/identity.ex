defmodule Janitor.Identity do
      ...
      "sub_" <>
        Base.encode16(
          :crypto.mac(
            ...
            Jason.encode!(["janitor-subject-v1", Keyword.fetch!(opts, :namespace), id])
          ),
          case: :lower
        )
  ...
  def create_delegation(c, input) do
    ...
    unless is_integer(expires) and expires > Janitor.now() and
             expires <= Janitor.now() + 30 * 86_400_000,
           do: raise(ArgumentError, "delegation must expire within 30 days")

    scopes = scopes!(input.scopes, 1)

    record = %{
      "id" => Janitor.random_id("dlg_"),
    ...
    if context[:delegation_id] do
      ...
      reason =
        cond do
          ...
          grant["expiresAt"] <= Janitor.now() -> "expired"
