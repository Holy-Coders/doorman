defmodule Doorman.Learning do
  ...
  defp scope(c), do: Doorman.Identity.label(c.identity, "doorman-learning-scope-v1")
  defp cutoff(c), do: Doorman.now() - (c.learning[:retention_days] || 30) * 86_400_000
  ...
  def observe(c, id, context, identity, payload) do
    ...
    if not allowed do
      ...
      if context[:verified] do
        ...
        observation = Doorman.Observation.normalize(payload["signals"], payload["behavior"])
        ...
        id = if reuse, do: id, else: Doorman.random_id("ses_")
        ...
        {id, max(0, div(expires - Doorman.now(), 1000)), prediction}
      end
    end
  end
        ...
        result =
          if is_function(opts[:predict], 1) do
            ...
            Doorman.Protection.evaluate(
              ...
              fn -> Doorman.Jev.predict_identity(input, c.evaluator) end,
              &is_map/1
            )
          end
