defmodule Doorman.Protection do
  ...
  def configure(opts) do
    Doorman.Identity.validate_options!(opts)
  ...
  def evaluate(c, fun, validate \\ &Doorman.Engine.valid_evaluation?/1, cost \\ 1)

  def evaluate(%{protection: nil} = c, fun, _validate, _cost),
    do: Doorman.Bounded.run(fun, c.evaluator_timeout_ms)
  ...
  def evaluate(c, fun, validate, cost) do
    ...
    case reservation do
      ...
      {:lease, lease} ->
        result = Doorman.Bounded.run(fun, c.evaluator_timeout_ms, true)
  ...
  defp key(c, value),
    do: Doorman.Identity.label(c.protection, Jason.encode!(["protection-v1", value]))
