defmodule DoormanTest do
  ...
  import Plug.Test
  import Plug.Conn
  alias Doorman.{Observation, Identity, Learning, Analytics}
  ...
  test "experimental fields validate, survive normalization and reach compact Jev state" do
    ...
    assert :ok == Doorman.Validation.validate(%{"signals" => raw})

    assert {:error, :invalid_payload} ==
             Doorman.Validation.validate(%{
    ...
    assert Doorman.Jev.compact(normalized)["fonts"] == fonts
  ...
  setup do
    Ecto.Adapters.SQL.query!(
      Doorman.TestRepo,
      "TRUNCATE doorman_test.visitors, doorman_test.identity_subjects, doorman_test.learning_sessions CASCADE",
      [],
      log: false
    )
  ...
  test "identity excludes automation and behavior while risk keeps them" do
    ...
    assert Doorman.Jev.input(input) == Doorman.Jev.input(altered)
    refute Doorman.Jev.risk_input(base) == Doorman.Jev.risk_input(changed)
    refute Map.has_key?(Doorman.Jev.risk_input(base)["state"], "history")
  end
        ...
        do:
          assert(
            %{"visitorId" => ^id, "isReturning" => true} =
              body(Doorman.handle(req(@signals, cookie), c))
          )

    Doorman.delete_visitor(c, id)
    refute body(Doorman.handle(req(@signals, "__visitor=" <> id), c))["isReturning"]
  end
  ...
  test "contradictions and sparse evidence cannot be amplified by Jev" do
    ...
    first = body(Doorman.handle(req(), c))
    other = body(Doorman.handle(req(Enum.at(@fixtures["vectors"], 6)["raw"]), c))
    refute first["visitorId"] == other["visitorId"]
    refute body(Doorman.handle(req(%{}), c))["isReturning"]
  end

  test "ambiguity does not restore an arbitrary device", %{c: c} do
    for _ <- 1..2 do
      id = Doorman.Storage.create(c)
      Doorman.Storage.save(c, id, Observation.normalize(@signals))
    end
  ...
  for mode <- [:timeout, :throw, :malformed] do
    test "evaluator #{mode} falls back" do
      ...
      result = body(Doorman.handle(req(), c))
  ...
  test "Jev plans lookup, batches candidates and predicts from verified flows without a custom callback" do
    ...
    for _ <- 1..2 do
      anon = Doorman.handle(req(), c)
      assert anon.status == 200

      assert Doorman.handle(req(@signals, learning_cookie(anon)), c, %{
               verified: %{subject_id: owner["id"], actor_id: owner["id"]}
             }).status == 200
    end

    next = Doorman.handle(req(), c)

    assert next.assigns.doorman_learning == %{
             "status" => "suggested",
             "subjectId" => owner["id"],
             "score" => 0.97
           }

    assert next.assigns.doorman_identity["attribution"]["subject"]["status"] == "unknown"
  ...
  test "HTTP boundary rejects claims, size abuse and foreign origins", %{c: c} do
    for extra <- [%{"verified" => %{}}, %{"learningConsent" => true}, %{"subjectId" => "someone"}],
        do: assert(Doorman.handle(req(@signals, "", extra), c).status == 400)

    assert Doorman.handle(req(%{"userAgent" => String.duplicate("x", 513)}), c).status == 400
  ...
  test "verified keys and scoped delegation" do
    ...
    result = body(Doorman.handle(req(), c, %{verified: ctx}))
  ...
  test "implementer chooses application or per-request collection policy" do
    ...
    assert Doorman.handle(req(), c).resp_cookies["__visitor_learning"] == nil
    ...
    assert Doorman.handle(req(@signals, learning_cookie(anon)), c, %{learning_consent: false}).status ==
             200
  ...
  test "conflicting verified accounts never become learning examples" do
    ...
    for id <- ~w(one two) do
      ...
      assert Doorman.handle(req(@signals, learning_cookie(first)), c, %{
               verified: %{subject_id: person["id"], actor_id: person["id"]}
             }).status == 200
    end

    assert Learning.reports(c) == []
    assert %{has_more_expired: false} = Doorman.cleanup(c)
  end
  ...
  test "shadow predictions stay private and are removed from predictor evidence" do
    ...
    ctx = %{verified: %{subject_id: person["id"], actor_id: person["id"]}}
    first = Doorman.handle(req(), c)
    Doorman.handle(req(@signals, learning_cookie(first)), c, ctx)
    second = Doorman.handle(req(), c)
    refute second.resp_body =~ person["id"]
    Doorman.handle(req(@signals, learning_cookie(second)), c, ctx)
  ...
  for provider <- [:posthog, :mixpanel] do
    test "#{provider} exports allowlisted evidence and explicit profile updates", %{c: c} do
      ...
      identity =
        body(Doorman.handle(req(), c)) |> Map.put("debug", %{"collectedSignals" => @signals})
  ...
  test "candidate lookup, history and retention remain bounded", %{c: c} do
    ...
    for _ <- 1..15 do
      id = Doorman.Storage.create(c)
      Doorman.Storage.save(c, id, observation)
    end

    candidates = Doorman.Storage.candidates(c, observation)
    assert length(candidates) == 10
    assert Doorman.Storage.candidates(c, %{}) == []
    id = hd(candidates)["visitor_id"]
    for _ <- 1..15, do: Doorman.Storage.save(c, id, observation)
    assert length(Doorman.Storage.history(c, id)) == 5

    assert [%{"n" => 10}] =
             Doorman.Storage.query(
               c,
               "SELECT COUNT(*) AS n FROM doorman_test.observations WHERE visitor_id=$1",
               [id]
             )

    Doorman.Storage.query(
      c,
      "UPDATE doorman_test.observations SET seen_at=0 WHERE visitor_id=$1",
      [id]
    )

    assert Doorman.Storage.history(c, id) == []
    Doorman.cleanup(c)
  ...
  test "provider body limits, HTTP failures and analytics rejection are controlled" do
    ...
    assert body(Doorman.handle(req(), c))["riskStatus"] == "unavailable"
    ...
    assert {:error, :rejected} ==
             Analytics.capture(:mixpanel, body(Doorman.handle(req(), config())), "account",
               token: "test",
  ...
  test "learning does not label unknown, delegated or agent actors" do
    ...
    for actor <- [nil, family["id"], agent["id"]] do
      ...
      Doorman.handle(req(@signals, learning_cookie(anon)), c, %{
  ...
  test "production debug gates and telemetry exceptions cannot leak or break identity" do
    ...
    refute Map.has_key?(body(Doorman.handle(req(@signals, "", %{"debug" => true}), c)), "debug")
  end
  ...
  test "default response hides scores and debug but retains private server evidence" do
    ...
    conn = Doorman.handle(req(@signals, "", %{"debug" => true}), c)
    assert Enum.sort(Map.keys(body(conn))) == ["isReturning", "visitorId"]
    assert conn.assigns.doorman_identity["risk"]["automation"] == 0.93
    assert is_map(conn.assigns.doorman_identity["debug"])
    assert Doorman.handle(req(@signals, "", %{"exposeClientScores" => true}), c).status == 400
  end

  test "indexed retrieval finds older history through a crowded coarse bucket", %{c: c} do
    {:ok, target} = Doorman.identify(c, %{"signals" => @signals})

    Doorman.Storage.query(c, "UPDATE doorman_test.observations SET seen_at = $1", [
      Doorman.now() - 86_400_000
    ])
    ...
    for _ <- 1..110 do
      id = Doorman.Storage.create(c)
      Doorman.Storage.save(c, id, Observation.normalize(decoy))
    end
    ...
    for raw <- [
      ...
      [best | _] = Doorman.Storage.candidates(c, Observation.normalize(raw))
      assert best["visitor_id"] == target["visitorId"]
      refute best["lookup_saturated"]
    end

    histories = Doorman.Storage.histories(c, [target["visitorId"]])
    assert length(histories[target["visitorId"]]) == 1
    {:ok, restored} = Doorman.identify(c, %{"signals" => @signals})
  ...
  test "crowded indistinguishable profiles cannot be restored by a confident evaluator", %{c: c} do
    ...
    candidates = Doorman.Storage.candidates(c, Observation.normalize(@signals))
    assert length(candidates) == 10
